...
three homomorphic activation layers rather than by network transfer -- a
substantially larger overhead than depth-limited HE designs typically
...
Transmitting the 1.79 MB input ciphertext over even a modest 100 Mbps link
costs $144$ ms, and the TLS handshake a further few milliseconds -- together
under $0.03\%$ of the $534.1$ s compute time. This inverts the conclusion an
...
qualitative claim it illustrates -- amortized per-record cost improves with
batch fill, and a sparsely filled batch still pays the full per-ciphertext
cost (Section~\ref{sec:discussion}) -- follows directly from the measured
$B=512$ anchor and the SIMD packing design (Sec.~\ref{sec:ckks-depth}), not
from the extrapolated curve itself.}
...
construction -- it is architecturally orthogonal to the forward pass -- and
...
"which HE configuration is better" for this network -- the two are not
...
of ciphertext storage, unchanged from the design estimate since ciphertext
size depends only on the CKKS parameters. Compute is a different story: at
...
privacy-compliant healthcare ML inference without data decryption, implemented
it end to end -- real CKKS encryption and homomorphic inference via Microsoft
SEAL, not a numerical simulation -- and introduced the \HEPI{} metric for
principled evaluation of HE-based clinical AI deployments. On a synthetic
cohort calibrated to published MIMIC-III statistics for in-hospital mortality
prediction, \framework{} attains an AUROC of $0.8622$ under full CKKS
encryption against $0.8607$ in plaintext on the same trained model -- a
statistically negligible gap, meaningfully better than an earlier,
