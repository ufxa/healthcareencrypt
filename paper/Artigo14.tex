...
conditions, \framework{} attains an AUROC of 0.862 (accuracy 89.3\%) under
full homomorphic encryption against 0.861 (89.1\%) in plaintext, a gap that is
statistically negligible, yielding a mean \HEPI{} of 0.830.
% [Weaker result]
...
activation degree, and Microsoft SEAL implements no CKKS bootstrapping, so
...
a $+0.0015$ difference that is within run-to-run noise (plaintext AUROC across
10 independently trained models varies by $\pm 0.0009$ SD), i.e., encryption
imposes no measurable AUROC cost for this architecture once the CKKS
parameter set has adequate multiplicative depth (Sec.~\ref{sec:ckks-depth}).
This is a materially stronger result than an earlier, unimplemented estimate
...
proxy are statistically indistinguishable on this run: $0.8622$ vs.\
$0.8623$ AUROC, a $-0.0001$ difference in \framework{}'s favor that is well
inside plaintext's own $\pm 0.0009$ seed-to-seed SD. An unimplemented estimate
of this pipeline had projected \framework{} exceeding the Lee et al.\ proxy by
$0.033$ AUROC; the measured result does not support that margin, and we
...
not a measured accuracy advantage on this cohort. Third, calibration varies
...
Table~\ref{tab:auroc}). \framework{} and the Lee et al.\ proxy are
statistically indistinguishable ($89.3\%$ each); the majority-class baseline
...
neither is plotted as a HEPI point here. Degree 3 and degree 7 are
statistically indistinguishable ($0.8297$ both, to four decimal places) --
...
pairing exists between differently-seeded models) gives $U = 100$,
...
methods are indistinguishable across essentially the entire simplex, not
one dominating the other. \framework{} scores (nominally) higher in $58$ of
...
robust $230/231$ win for \framework{}; the measured comparison is a
statistical tie throughout the weight simplex, and we report that finding
rather than the earlier projection. This sweep therefore does not answer
"which HE configuration is better" for this network, since the two are not
measurably different; it answers "how sensitive is that non-finding to the
weight choice," and the answer is: not sensitive, because there is no
detectable difference to be sensitive to.
...
remains within an overnight batch window on dedicated hardware, and scales
down roughly linearly with additional parallel cores (our own throughput
improved when we reduced worker count to better fit available, contended CPU
capacity, Sec.~\ref{sec:ckks-depth}, so the relationship between core
count and throughput on an uncontended machine remains to be characterized
precisely), but it is a genuine infrastructure requirement, not the
...
and the Lee et al.\ proxy remain statistically indistinguishable across the
entire weight simplex, so at minimum the metric does not manufacture a
comparative ranking where the underlying measurement supports none. More
seriously, the ablation study demonstrates that \HEPI{} is blind to
...
encryption against $0.8607$ in plaintext on the same trained model, a
statistically negligible gap and meaningfully better than an earlier,
unimplemented estimate of this pipeline had assumed, with a measured mean
\HEPI{} of $0.830$. \framework{} and the Lee et al.\ proxy baseline are
statistically indistinguishable on this measurement ($0.8622$ vs.\ $0.8623$
AUROC, identical \HEPI{} to four decimal places); \framework{}'s contribution
over that baseline is architectural and regulatory, not a measured accuracy
...
32, and NIST SP 800-111 that the Lee et al.\ proxy does not.
